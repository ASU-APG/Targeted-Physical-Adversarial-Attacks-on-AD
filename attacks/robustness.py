...
def get_obj_params_robust(car_props, object_true_loc, robust_vec):
    ...
    object_true_loc = [(x - robust_vec[0].item(), y - robust_vec[1].item())
                       for (x, y) in object_true_loc]
...
def run_robustness_test(env, agent, d_s, target, limits, start_pos, T):
    ...
    for x in range(x_m_dir, x_p_dir + 1):
        ...
        for y in range(y_m_dir, y_p_dir + 1):
            _, _, _, _, s_loss_attack = run_env_robust(
                env, agent, d_s, target, start_pos, T, torch.tensor([x, y]))
...
def plot_robustness(heatmap_mat, limits, T, eps, save):
    ...
    im = ax.imshow(heatmap_mat, interpolation='none',
...
def main():
    ...
    _, _, _, _, s_loss_attack_start = run_env_robust(
        env, agent, d_s, target, start_pos, T)

    # run robustness experiment
    x_m_dir, x_p_dir = -9, 2  # exceeding this can make box go out of track
    y_m_dir, y_p_dir = -9, 2
    heatmap_mat = run_robustness_test(env, agent, d_s, target, [(
        x_m_dir, x_p_dir), (y_m_dir, y_p_dir)], start_pos, T)
    plot_robustness(
        heatmap_mat, [(x_m_dir, x_p_dir), (y_m_dir, y_p_dir)], T, eps, args.save)
    print(
        f'Robustness plot showed/saved for scenario {args.scenario}, T = {T}, and eps = {eps}')
