...
class CarRacingAdv(gym.Env, EzPickle):
    ...
    def _create_track(self):
        ...
        for i in range(len(track)):
            ...
            # this is responsible for generating the track
    ...
    def step(self, action):
        ...
        self.adv_state = self.render("state_pixels", adv=True)
        ...
        x, y = self.car.hull.position
        linVelx, linVely = self.car.hull.linearVelocity
        car_props = [(x, y), self.car.hull.angle, (linVelx, linVely), self.car.hull.angularVelocity]

        return self.state, step_reward, done, {}, self.adv_state, car_props, self.obj_poly

    def render(self, mode='human', adv=False):
        ...
        t.enable()
        self.render_road(adv)
        for geom in self.viewer.onetime_geoms:
            geom.render()
        self.viewer.onetime_geoms = []
        t.disable()
        if not adv and self.show_indicators:
    ...
    def render_road(self, adv=False):
        if not adv:
...
if __name__ == "__main__":
    ...
    while isopen:
        ...
        restart = False
        while True:
            s, r, done, info, adv, car_props, obj_true_loc = env.step(a)
            # print(obj_true_loc)
            total_reward += r
