...
class _RolloutDataset(torch.utils.data.Dataset):  # pylint: disable=too-few-public-methods
    def __init__(self, root, transform, buffer_size=1000, train=True):  # pylint: disable=too-many-arguments
        ...
        split_idx = int(len(self._files) * 0.6)

        if train:
            self._files = self._files[:split_idx]
        else:
            self._files = self._files[split_idx:]
...
class RolloutSequenceDataset(_RolloutDataset):  # pylint: disable=too-few-public-methods
    ...
    def __init__(self, root, seq_len, transform, buffer_size=1000, train=True):  # pylint: disable=too-many-arguments
...
class RolloutObservationWithObjDataset(_RolloutDataset):
    def _data_per_sequence(self, data_length):
        return data_length

    def _get_data(self, data, seq_index):
        return self._transform(data['observations_with_obj'][seq_index])
