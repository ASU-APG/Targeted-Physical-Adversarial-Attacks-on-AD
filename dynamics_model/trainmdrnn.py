...
from dynamics_model.misc import ASIZE, LSIZE, RSIZE, RED_SIZE, SIZE
from dynamics_model.learning import EarlyStopping
import warnings
warnings.filterwarnings("ignore")

from dynamics_model.loaders import RolloutSequenceDataset
from dynamics_model.models.vae import VAE
from dynamics_model.models.mdrnn import MDRNN, gmm_loss

parser = argparse.ArgumentParser("MDRNN training")
parser.add_argument('--logdir', default='dynamics_model/params', type=str,
...
# Loading model
rnn_dir = join(args.logdir, 'mdrnn_' + args.dataset)
...
train_loader = DataLoader(
    RolloutSequenceDataset(dataset, SEQ_LEN, transform, buffer_size=1000),
    batch_size=BSIZE, num_workers=8, shuffle=True)
test_loader = DataLoader(
    RolloutSequenceDataset(dataset, SEQ_LEN, transform, train=False, buffer_size=1000),
...
def to_latent(obs, next_obs):
    ...
    with torch.no_grad():
        ...
        latent_obs, latent_next_obs = [
            (x_mu + x_logsigma.exp() * torch.randn_like(x_mu)).view(bs, sl, LSIZE)
